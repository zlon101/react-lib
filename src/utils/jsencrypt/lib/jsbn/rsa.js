...
function pkcs1pad1(s, n) {
  ...
  let len = n - s.length - 6;
  let filler = '';
  for (let f = 0; f < len; f += 2) {
    filler += 'ff';
  }
  let m = '0001' + filler + '00' + s;
...
let RSAKey = /** @class */ (function() {
  ...
  RSAKey.prototype.sign = function(text, digestMethod, digestName) {
    let header = getDigestHeader(digestName);
    let digest = header + digestMethod(text).toString();
    let m = pkcs1pad1(digest, this.n.bitLength() / 4);
    if (m == null) {
      return null;
    }
    let c = this.doPrivate(m);
    if (c == null) {
      return null;
    }
    let h = c.toString(16);
  ...
  RSAKey.prototype.verify = function(text, signature, digestMethod) {
    let c = parseBigInt(signature, 16);
    let m = this.doPublic(c);
    if (m == null) {
      return null;
    }
    let unpadded = m.toString(16).replace(/^1f+00/, '');
    let digest = removeDigestHeader(unpadded);
...
function removeDigestHeader(str) {
  for (let name_1 in DIGEST_HEADERS) {
    if (DIGEST_HEADERS.hasOwnProperty(name_1)) {
      let header = DIGEST_HEADERS[name_1];
